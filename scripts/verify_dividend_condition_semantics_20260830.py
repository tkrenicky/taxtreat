...
VOTING_MARKERS = ("hlasovac", "voting")
DIRECT_MARKERS = ("přímo", "primo", "directly", "direct ")
COMPANY_MARKERS = (
...
def _normalized_source(rule: dict) -> str:
    return re.sub(r"\s+", " ", str(rule.get("source_text") or "").lower())
    ...
    for path in sorted(RULE_DIR.glob("*.json")):
        ...
        for raw_rule in package.get("rules", []):
            ...
            if source_mentions_company and "recipient_entity_type" not in facts:
                failures.append(f"{country} {rule.get('rule_id')}: source text limits the reduced dividend branch to a company but rule has no recipient_entity_type condition")
    if failures:
        raise AssertionError("Dividend condition semantic failures:\n" + "\n".join(sorted(set(failures))))
    print("Dividend condition semantics after runtime remediation: PASS")
    return 0
