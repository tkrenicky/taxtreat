...
def main() -> int:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            page.wait_for_function("() => document.querySelector('[data-view=\"payers\"]')?.classList.contains('active')")
            ...
            page.locator('button[data-nav="dashboard"]:visible').first.click()
            page.locator('[data-start-flow]:visible').first.click()
            page.locator('.flow-step[data-step="1"] [data-next-step="2"]').click()
            page.locator("[data-show-recipient-form]:visible").click()
            ...
            page.wait_for_timeout(1000)
            treaty_card = result.locator('#workspace-citations .citation-card', has_text="APPLIED TREATY RULE").first
    ...
    print("Workspace profile and dividend fallback acceptance v2: PASS")
    return 0
