...
def _extract_flexible(text: str, article: str, rules: list[dict]) -> str | None:
    ...
    for match in matches:
        ...
        next_match = re.search(r"(?i)\barticle\s+(?:no\.?\s*)?(?:[0-9]{1,2}|[IVXLC]{1,8})\b", tail[max(1, match.end() - match.start()):])
        end = len(tail)
        if next_match:
            end = max(1, match.end() - match.start()) + next_match.start()
        ...
        if subject and subject in lowered:
            score += 10
        if core._english_likelihood(excerpt):
            score += 5
...
def _process(country: str, articles: dict[str, list[dict]], entry: dict | None) -> tuple[dict, dict | None]:
    ...
    row = {
        "country": country,
        "status": "NO_REGISTERED_SOURCE" if not sources else "NO_EN_TEXT",
        "source_attempts": [],
        "articles": {a: {"expected_rates": core._expected_rates(r), "status": "NO_EN_ARTICLE", "excerpt_length": 0, "missing_rates": core._expected_rates(r)} for a, r in articles.items()},
    }
    best_locales: dict[str, dict] = {}
    any_pair_valid = False
    any_source_ok = False
    for source in sources:
        url = str(source.get("url"))
        attempt = {"url": url, "authority": source.get("authority"), "status": "SOURCE_ERROR", "articles": {}}
        try:
            body, _, resolved = core._request(url, timeout=25)
            if not _same_host_family(url, resolved):
                raise RuntimeError(f"redirect outside registered authority: {resolved}")
        ...
        for article, rules in articles.items():
            if article not in allowed:
                continue
            candidate, excerpt = _analyse(text, article, rules)
            ...
            if STATUS_RANK[candidate["status"]] > STATUS_RANK[row["articles"][article]["status"]]:
                ...
                if excerpt:
                    best_locales[article] = {"en": {"language": "en", "status": "candidate_official_treaty_text", "authority": source.get("authority"), "source_url": resolved, "text": excerpt}}
        row["source_attempts"].append(attempt)
    ...
    if best_locales:
        candidate_payload = {"schema_version": 1, "source_country": "CZ", "recipient_country": country, "candidate_only": True, "articles": best_locales}
    return row, candidate_payload


def main() -> int:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    registry = _load(REGISTRY).get("entries") or {}
    ...
    eligible = [c for c in countries if c in registry]
    print(f"Explicit official EN treaty extraction v9: missing={len(grouped)} pairs, registered_missing={len(eligible)} countries", flush=True)
    results = []
    ...
    counts: dict[str, int] = {}; passes = reviews = 0
    for row in results:
    ...
    print(f"v9 counts: {json.dumps(counts, sort_keys=True)}", flush=True); print(f"Article PASS={passes} REVIEW={reviews}", flush=True)
    return 0
