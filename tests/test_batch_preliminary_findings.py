...
def test_registry_contains_four_findings() -> None:
    payload = build_findings()

    assert payload["summary"]["total_scopes"] == 30
    assert payload["summary"]["preliminary_findings_completed"] == 4
    assert payload["summary"]["preliminary_completion_percent"] == 13.3
