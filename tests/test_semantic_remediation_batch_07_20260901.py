...
BATCH = ("QA", "SI", "SK", "TN", "TW")

def test_semantic_remediation_batch_07_is_hash_bound_and_fail_closed():
    queue = json.loads(QUEUE.read_text(encoding="utf-8"))
    hashes = {str(row["partner_country"]).upper(): str(row["package_sha256"]) for row in queue["packages"]}
    for country in BATCH:
        payload = json.loads((CANDIDATES / f"{country.lower()}.json").read_text(encoding="utf-8"))
        ...
        assert production["verification_authority"] == "semantic_remediation_machine_projection"
        remediated = [rule for rule in payload["rules"] if rule.get("review_package_sha256") == hashes[country] and rule.get("verification_authority") == "semantic_remediation_machine_projection"]
