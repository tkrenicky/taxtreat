...
def test_category_sensitive_reconciliation_queue_is_exactly_the_reviewed_20_country_set():
    audit = _audit()
    ...
    assert all(
        row["category_projection_review_required"]
        == (row["scope_key"][1] in CATEGORY_SENSITIVE_REVIEW_COUNTRIES)
...
def test_machine_audit_never_treats_keyword_or_percentage_flags_as_legal_projection():
    audit = _audit()
