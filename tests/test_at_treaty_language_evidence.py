...
from taxtreat.tools.build_at_language_evidence import build_language_evidence, classify_source_language
...
def _fixture(tmp_path: Path):
    ...
    landing.write_text(
        '<html><body>'
        '<a href="https://www.ris.bka.gv.at/Dokumente/BgblAuth/example/deutsch.pdf" title="Signiertes PDF-Dokument: deutscher Vertragstext"></a>'
        '<a href="https://www.ris.bka.gv.at/Dokumente/BgblAuth/example/english.pdf" title="Signiertes PDF-Dokument: englischer Vertragstext"></a>'
        '</body></html>',
        encoding="utf-8",
    )
    ...
    partners = [
        ...
        {
            "partner_label": "Partner 2",
            "sources": [{
                "source_order": 1,
                "final_url": "https://www.ris.bka.gv.at/eli/bgbl/III/2020/2/20200101",
                "role_candidate": "published_instrument_or_protocol",
                "content_type": "application/pdf",
                "sha256": "unknown",
                "artifact_path": str(german),
    ...
    article_partners = [
        {
            "partner_label": "Partner 0",
            "sources": [
                {"source_sha256": "de-source", "article_candidates": [{
                    "article_number": 12,
                    ...
                    "semantic_income_detected": "royalty",
                }]},
                {"source_sha256": "en-source", "article_candidates": [{
                    "article_number": 12,
        ...
        {
            "partner_label": "Partner 1",
            "sources": [{"source_sha256": "current", "article_candidates": [{
                "article_number": 12,
        ...
        {"partner_label": "Partner 2", "sources": []},
    ]
    pilot = {
        "source_country": "AT",
        "acquisition_scope": "all_current_partners",
        "pilot_partner_count": len(partners),
        "partners": partners,
    }
    articles = {
        "source_country": "AT",
        "partner_count": len(article_partners),
...
def test_at_language_evidence_tracks_german_and_english_without_releasing_wording(tmp_path: Path):
    ...
    assert evidence["partner_count"] == 3
    assert evidence["policy"]["universe_size_is_derived_from_current_acquisition_not_hard_coded"] is True
    ...
    assert all(item["semantic_income_detected"] == "royalty" for item in row["article_language_evidence"])
...
def test_at_current_consolidated_view_is_german_candidate_not_authenticity_conclusion(tmp_path: Path):
    pilot, articles = _fixture(tmp_path)
...
def test_at_language_evidence_requires_matching_full_current_universe(tmp_path: Path):
    pilot, articles = _fixture(tmp_path)
    pilot["pilot_partner_count"] = 2
    with pytest.raises(ValueError, match="universe mismatch"):
        build_language_evidence(pilot, articles, artifact_root=tmp_path)

    pilot, articles = _fixture(tmp_path)
    pilot["acquisition_scope"] = "selected_partners"
    with pytest.raises(ValueError, match="full current-partner acquisition scope"):
        build_language_evidence(pilot, articles, artifact_root=tmp_path)

    pilot, articles = _fixture(tmp_path)
    articles["partners"] = articles["partners"][1:]
    with pytest.raises(ValueError, match="partner labels do not match"):
        build_language_evidence(pilot, articles, artifact_root=tmp_path)


def test_at_language_evidence_rejects_wrong_source_country(tmp_path: Path):
    pilot, articles = _fixture(tmp_path)
...
def test_at_language_classifier_handles_redirected_attachment_label_and_url_marker(tmp_path: Path):
    landing = tmp_path / "landing.html"
    landing.write_text('<a href="https://www.ris.bka.gv.at/source/english.pdf" title="englischer Vertragstext">English</a>', encoding="utf-8")
    parent = {
        "final_url": "https://www.ris.bka.gv.at/eli/bgbl/III/2020/1/20200101",
        "content_type": "text/html",
        "artifact_path": str(landing),
    ...
    ) == ("en", "ris_attachment_label")
    direct = {"final_url": "https://www.ris.bka.gv.at/files/german-treaty-text.pdf", "role_candidate": "official_text_attachment"}
    assert classify_source_language(direct, sources_by_url={}, artifact_root=tmp_path) == ("de", "url_language_marker")


def test_at_language_classifier_falls_back_when_parent_evidence_is_unusable(tmp_path: Path):
    ...
    assert classify_source_language(missing_parent_source, sources_by_url={}, artifact_root=tmp_path) == ("unknown", "not_determined")


def test_at_language_evidence_cli_writes_fail_closed_inventory(tmp_path: Path, monkeypatch):
    pilot, articles = _fixture(tmp_path)
