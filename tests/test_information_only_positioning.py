...
def test_public_ui_uses_information_only_positioning():
    ...
    assert "neposkytuje individuální právní ani daňové poradenství" in legacy_html.lower()
    assert "Informační nástroj" in workspace_html
    assert "neposkytuje individuální právní ani daňové poradenství" in workspace_html.lower()
