...
def test_at_audit_flags_multiple_rate_category_split(tmp_path: Path):
    ...
    assert row["rate_tokens_machine"] == [5.0, 10.0]
    assert "multiple_numeric_rate_tokens" in row["machine_risk_reasons"]
...
def test_at_audit_flags_ownership_condition_as_possible_non_category_rate(tmp_path: Path):
    audit = build_audit(_inventory(tmp_path), artifact_root=tmp_path)
    row = audit["partners"][2]
    assert row["keyword_flags"]["ownership_condition"] is True
    assert "multiple_rates_may_be_non_category_ownership_condition" in row["machine_risk_reasons"]
