(() => {
  ...
  const EN = new Map([
    ...
    ["Možné vnitrostátní osvobození", "Potential domestic exemption"], ["Vnitrostátní osvobození podle zákona o daních z příjmů", "Domestic exemption under the Czech Income Taxes Act"],
  ]);
  ...
  function renderSection19Result() {
    ...
    if (state.uiLanguage === "en") {
      if (finalExemption || applicable) box.innerHTML = '<h2>Domestic exemption under the Czech Income Taxes Act</h2><p><strong>Applicable – Czech withholding tax is not due.</strong> The domestic exemption under Section 19 of the Czech Income Taxes Act is the primary legal basis. Treaty treatment is supplementary.</p>';
      else if (allNotApplicable) box.innerHTML = '<h2>Domestic exemption under the Czech Income Taxes Act</h2><p>The domestic exemption was assessed first and is not available based on the entered facts. The treaty analysis therefore determines the withholding tax treatment.</p>';
      else if (unresolved || !analysis) box.innerHTML = '<h2>Domestic exemption under the Czech Income Taxes Act</h2><p><strong>Not yet resolved.</strong> Section 19 is assessed before treaty relief. Complete the factual Section 19 questions in the payment step; any remaining legal qualification stays an internal TaxTreat review item rather than a question requiring the payer to interpret Czech tax law.</p>';
      else box.innerHTML = '<h2>Domestic exemption under the Czech Income Taxes Act</h2><p>Section 19 was assessed before treaty relief. The treaty result shown below applies only if the domestic exemption is not available.</p>';
    } else {
      if (finalExemption || applicable) box.innerHTML = '<h2>Vnitrostátní osvobození podle zákona o daních z příjmů</h2><p><strong>Osvobození se použije – česká srážková daň se neodvádí.</strong> Primárním právním titulem je § 19 ZDP; smluvní režim je pouze doplňkový.</p>';
      else if (allNotApplicable) box.innerHTML = '<h2>Vnitrostátní osvobození podle zákona o daních z příjmů</h2><p>Osvobození bylo posouzeno jako první a podle zadaných údajů se neuplatní. Daňové zacházení proto určuje smluvní analýza.</p>';
      else if (unresolved || !analysis) box.innerHTML = '<h2>Vnitrostátní osvobození podle zákona o daních z příjmů</h2><p><strong>Zatím nelze uzavřít.</strong> § 19 se posuzuje před smluvní úlevou. Doplň jednoduché skutkové otázky k § 19 v kroku platby; případná zbývající právní kvalifikace zůstane interním bodem pro TaxTreat a nebude přenesena na plátce.</p>';
      else box.innerHTML = '<h2>Vnitrostátní osvobození podle zákona o daních z příjmů</h2><p>§ 19 byl posouzen před smluvní úlevou. Smluvní výsledek uvedený níže se použije pouze tehdy, pokud vnitrostátní osvobození není dostupné.</p>';
    }
  }
