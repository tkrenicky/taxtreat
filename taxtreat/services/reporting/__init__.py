...
def _legal_reference(report, source):
    ...
    if source.get("legal_layer") == "treaty":
        return f"{ref} {_treaty_name(report)}"
    if source.get("legal_layer") == "protocol":
        return f"{ref} protokolu k {_treaty_name(report)}"
...
def render_report_html(report):
    ...
    article_heading = f"Článek {escape(str(selected.get('article') or '—'))}" if selected else "Právní ustanovení"
    ...
    path_note = (
        ...
        + f" → {rate_display} podle {_article(treaty) if treaty else _article(selected) if selected else 'použitého ustanovení'} {treaty_name}."
    )
