...
def render_report_html(report):
    ...
    rate_display, _, conclusion = _result_copy(result, selected)
    if result.get("status") != "FINAL":
        conclusion = "Zadané údaje zatím neumožňují určit konkrétní sazbu nebo režim. Údaje, které je třeba doplnit, jsou uvedeny níže."
    ...
    selected_excerpt = escape(_short_excerpt(selected, 2500)) if selected else "Právní výňatek není k dispozici."
    ...
    compatibility = (
        ...
        '<table hidden><tbody>'
        f'<tr><th>{tax_label}</th><td>{calc_tax}</td></tr>'
        '</tbody></table>'
    )
...
<style>
:root{{--navy:#102150;--blue:#1557d6;--cream:#fff7e5;--pale:#eef4ff;--green:#178447;--green-bg:#eef8f1;--ink:#15191f;--text:#3d4657;--muted:#748097;--line:#dfe5ed}}
*{{box-sizing:border-box}}html,body{{margin:0;background:#f2f5f9;color:var(--text);font-family:Inter,Arial,"Segoe UI",sans-serif}}a{{color:var(--blue);text-decoration:none}}.report{{width:210mm;margin:14px auto}}.page{{position:relative;width:210mm;height:297mm;padding:6mm;background:#f2f5f9;page-break-after:always;overflow:hidden}}.page:last-child{{page-break-after:auto}}.sheet{{position:relative;height:100%;padding:9mm 10mm 10mm;border:1px solid #e6e1d7;border-radius:6mm;background:#fffdf9;overflow:hidden;box-shadow:0 12px 34px #18315d0d}}
.header{{height:10mm;display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4mm}}.brand{{display:flex;align-items:center;gap:6px;color:var(--navy);font:700 17px/1 Georgia,"Times New Roman",serif;letter-spacing:-.03em}}.shield{{display:grid;place-items:center;width:21px;height:21px;border:2px solid var(--blue);border-radius:7px;color:var(--blue);font:800 7px/1 Arial}}.head-meta{{text-align:right;color:var(--muted);font-size:7.2px;line-height:1.4}}.head-meta b{{display:block;color:var(--ink);font-size:8.5px}}
.hero{{display:grid;grid-template-columns:1.35fr .65fr;gap:7mm;align-items:center;margin:0 -10mm 5mm;padding:7mm 10mm;background:linear-gradient(105deg,#fff1cf 0%,#fff8e8 63%,#eef4ff 100%)}}.hero h1{{margin:1.8mm 0;color:var(--ink);font:700 25px/1.08 Georgia,"Times New Roman",serif;letter-spacing:-.035em}}.hero p{{margin:0;max-width:115mm;color:#667189;font-size:8.4px;line-height:1.45}}.hero-art svg{{display:block;width:100%;max-height:34mm}}.kicker{{display:block;color:var(--blue);font-size:6.9px;font-weight:800;letter-spacing:.1em;text-transform:uppercase}}
h2,h3{{margin:0;color:var(--ink);font-family:Georgia,"Times New Roman",serif;letter-spacing:-.02em}}h2{{font-size:16px}}h3{{font-size:11px}}p{{font-size:8px;line-height:1.45}}
.summary-grid{{display:grid;grid-template-columns:1.08fr .92fr;gap:4mm}}.card{{border:1px solid var(--line);border-radius:4mm;background:#fff;padding:4.2mm}}.result-card{{background:#fff9eb;border-color:#ecdfbf}}.facts-card{{background:#f3f7ff;border-color:#dfe8fb}}.rate-row{{display:flex;align-items:flex-start;justify-content:space-between;gap:4mm}}.rate{{margin:1.8mm 0;color:var(--green);font-size:35px;font-weight:800;letter-spacing:-.05em}}.status-note{{padding:2mm 2.8mm;border-radius:999px;background:#fff3d8;color:#96600d;font-size:6.8px;font-weight:800}}.basis-row,.fact-row{{display:grid;grid-template-columns:1fr auto;gap:4mm;padding:2.2mm 0;border-top:1px solid var(--line);font-size:7.8px}}.basis-row span,.fact-row span{{color:var(--muted)}}.basis-row b,.fact-row b{{max-width:82mm;color:var(--ink);text-align:right}}.conclusion{{margin-top:3mm;padding:3.2mm 3.5mm;border:1px solid #b9dbc3;border-radius:3mm;background:var(--green-bg);color:#2b6840;font-size:7.8px;line-height:1.45}}
.assumptions{{margin-top:4mm;padding:4mm;border:1px solid var(--line);border-radius:4mm;background:#fff}}.assumptions-head{{display:flex;justify-content:space-between;align-items:baseline;gap:5mm;margin-bottom:2mm}}.assumptions-head p{{margin:0;color:var(--muted);font-size:7.2px}}.assumptions-grid{{display:grid;grid-template-columns:1fr 1fr;column-gap:7mm}}.assumption-row{{display:flex;justify-content:space-between;gap:4mm;padding:1.8mm 0;border-top:1px solid var(--line);font-size:7.4px}}.assumption-row span{{color:var(--muted)}}.assumption-row b{{color:var(--ink);text-align:right}}
.analysis-block{{display:grid;grid-template-columns:1.15fr .85fr;gap:4mm;margin-top:4.5mm}}.decision-card{{padding:4.2mm;border-radius:4mm;background:#eef4ff}}.decision-intro{{margin:1mm 0 3mm;color:var(--muted);font-size:7.2px}}.decision-step{{display:grid;grid-template-columns:6.5mm 1fr;gap:2.2mm;padding:2.3mm 0;border-top:1px solid #dce6f7}}.decision-step>span{{display:grid;place-items:center;width:5.5mm;height:5.5mm;border:1px solid #86b39a;border-radius:50%;color:var(--green);background:#fff;font-size:6.8px;font-weight:900}}.decision-step b{{color:var(--ink);font-size:7.8px}}.decision-step p{{margin:.7mm 0 0;color:var(--muted);font-size:7.1px;line-height:1.4}}.calc-card{{padding:4.2mm;border:1px solid var(--line);border-radius:4mm;background:#fff}}.calc-row{{display:flex;justify-content:space-between;gap:4mm;padding:2.6mm 0;border-bottom:1px solid var(--line);font-size:7.8px}}.calc-row:last-of-type{{border-bottom:0}}.calc-row span{{color:var(--muted)}}.calc-row b{{color:var(--ink)}}.fx{{margin-top:2.5mm;padding-top:2.5mm;border-top:1px solid var(--line);color:var(--muted);font-size:6.8px;line-height:1.4}}
.section-head{{display:grid;grid-template-columns:1fr 38mm;gap:7mm;align-items:center;margin:0 -3mm 4.5mm;padding:5mm 5mm;border-radius:4mm;background:#fff1d3}}.section-head p{{margin:1.2mm 0 0;color:var(--muted);font-size:7.4px}}.section-head svg{{width:100%;height:22mm}}.legal-basis-kicker{{margin:2mm 0 1.5mm}}.legal-source{{border:1px solid var(--line);border-radius:4mm;padding:4.5mm;background:#fff}}.legal-title-row{{display:flex;justify-content:space-between;gap:6mm;align-items:flex-start}}.legal-title-row h2{{font-size:15px}}.official{{flex:0 0 auto;font-size:7.2px;font-weight:750;white-space:nowrap}}.legal-summary{{margin:1.2mm 0 2.5mm;color:var(--muted);font-size:7.3px}}.quote{{padding:4mm;border-radius:3mm;background:#f7f4ee;color:#434b62;font-size:7.6px;line-height:1.48;white-space:pre-line;max-height:108mm;overflow:hidden}}
.lower-grid{{display:grid;grid-template-columns:1.08fr .92fr;gap:4mm;margin-top:4.5mm}}.deadline-wrap,.support-wrap{{border:1px solid var(--line);border-radius:4mm;padding:4.2mm;background:#fff}}.deadline-grid{{display:grid;grid-template-columns:1fr 1fr;gap:2.5mm;margin-top:2.5mm}}.deadline-card{{padding:3.2mm;border-radius:3mm;background:#f3f7ff}}.deadline-card:nth-child(2n){{background:#fff5da}}.deadline-card span{{display:block;color:var(--muted);font-size:7px;line-height:1.35}}.deadline-card b{{display:block;margin-top:1.2mm;color:var(--ink);font-size:9.5px}}.support-grid{{display:grid;grid-template-columns:1fr 1fr;gap:3mm;margin-top:2.5mm}}.mini-card{{padding:3.2mm;border-radius:3mm;background:#fff9eb}}.mini-card:nth-child(2){{background:#f5f7fb}}.mini-card ul{{margin:1.8mm 0 0;padding-left:4.5mm}}.mini-card li{{margin-bottom:1.5mm;font-size:7px;line-height:1.35}}.related-sources{{margin-top:3.5mm;display:grid;grid-template-columns:repeat(2,1fr);gap:2.5mm}}.related-source{{padding:2.8mm;border:1px solid var(--line);border-radius:3mm;background:#fff}}.related-source span{{display:block;color:var(--muted);font-size:6.6px}}.related-source b{{display:block;margin:1mm 0;color:var(--ink);font-size:7.7px}}.related-source div{{font-size:6.8px}}.disclaimer{{margin-top:4mm;padding-top:3mm;border-top:1px solid var(--line);color:#858da0;font-size:6.5px;line-height:1.4}}.footer{{position:absolute;left:10mm;right:10mm;bottom:4.5mm;display:flex;justify-content:space-between;color:#99a1b1;font-size:6.4px}}.footer b{{color:#6e7790}}
