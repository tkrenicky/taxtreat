...
def build_review_pack(
    ...
    if income_type not in {"interest", "royalty"}:
        raise ValueError(
            f"Unsupported income type: {income_type}"
        )
    ...
    worksheet_scope = next(
        ...
        if scope["income_type"] == income_type
    )
    ...
    return {
        "schema_version": 1,
        "dataset_release": (
            f"batch-01-be-{income_type}-primary-review-pack-"
            "2026-08-06.1"
        ),
        ...
        "treaty": {
            ...
            "article_number": base_treaty.get(
                "article_number"
            ),
            "article_title": base_treaty.get(
                "article_title"
            ),
...
def output_paths(
    ...
    return (
        ...
        / (
            "batch_01_be_"
            f"{income_type}_primary_review_pack.json"
        ),
        base
        / (
            "batch_01_be_"
            f"{income_type}_primary_review_pack.md"
        ),
    )
...
def main() -> None:
    for income_type in ("interest", "royalty"):
        ...
        print(
            f"Belgium {income_type} primary-review "
            "pack created."
        )
        print(
            "JSON:",
